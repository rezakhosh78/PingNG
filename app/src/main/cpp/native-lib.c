...
// `--tls-sni` rewrites the process-global fake ClientHello template. Search
// starts many candidates in one Android process, so restore the immutable
// template before parsing every candidate; otherwise one SNI/length change
// leaks into the next candidate.
static char base_tls_data[sizeof(tls_data)];
...
JNIEXPORT jint JNI_OnLoad(
    ...
    memcpy(base_tls_data, tls_data, sizeof(tls_data));
...
Java_com_v2ray_ang_core_PingNGProxy_jniCreateSocketWithCommandLine(
        JNIEnv *env,
        __attribute__((unused)) jobject thiz,
        jobjectArray args) {
    // A search creates many proxy instances in one daemon process. Start each
    // candidate from the immutable defaults instead of inheriting the previous
    // method's desync_params/tlsrec arrays.
    memcpy(tls_data, base_tls_data, sizeof(tls_data));
    reset_params();
...
JNIEXPORT jint JNICALL
Java_com_v2ray_ang_core_PingNGProxy_jniCreateSocket(
        JNIEnv *env,
        __attribute__((unused)) jobject thiz,
        jstring ip,
        jint port,
...
JNIEXPORT jint JNICALL
Java_com_v2ray_ang_core_PingNGProxy_jniStartProxy(
...
JNIEXPORT jint JNICALL
Java_com_v2ray_ang_core_PingNGProxy_jniStopProxy(
        __attribute__((unused)) JNIEnv *env,
        __attribute__((unused)) jobject thiz,
        jint fd) {
