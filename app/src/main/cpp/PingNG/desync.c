...
/* send(2) is allowed to write less than requested, especially on Android's
 * non-blocking sockets. Keep the existing EAGAIN semantics while preventing
 * silent truncation of the last part of an upload. */
static ssize_t send_full(int sfd, const char *buffer, size_t size, int flags)
{
    size_t sent = 0;
    while (sent < size) {
        ssize_t n = send(sfd, buffer + sent, size - sent, flags);
        if (n < 0) {
...
static long random_between(long min, long max)
{
...
void wait_send(int sfd)
{
    ...
    delay(random_between(params.sfdelay_min, params.sfdelay_max));
}
        ...
        const char *selected_sni)
{
    ...
    char *p = 0;
    ssize_t len = -1;
    int fake_ttl = opt->ttl_min > 0
        ? (int)random_between(opt->ttl_min, opt->ttl_max)
        : (opt->ttl ? opt->ttl : 8);
    ...
    while (1) {
        ...
        char *payload_data = pkt.data;
        char temp_tls[sizeof(tls_data)];
        if (cnt != IS_HTTP && selected_sni && !opt->fake_data.data) {
            memcpy(temp_tls, fake_tls.data, fake_tls.size);
            change_tls_sni(selected_sni, temp_tls, fake_tls.size);
            if (opt->fake_jitter) {
                randomize_tls_padding(temp_tls, fake_tls.size);
            }
            payload_data = temp_tls;
        }

        if (cnt != IS_HTTP && !selected_sni && opt->fake_jitter && !opt->fake_data.data) {
            memcpy(temp_tls, fake_tls.data, fake_tls.size);
            randomize_tls_padding(temp_tls, fake_tls.size);
            payload_data = temp_tls;
        }

        memcpy(p, payload_data, pkt.size < pos ? pkt.size : pos);
        
        if (setttl(sfd, fake_ttl, fa) < 0) {
            break;
        }
        ...
        ssize_t n, long pos, char *c)
{
    ...
    ssize_t len = send_full(sfd, buffer, pos + 1, MSG_OOB);
    buffer[pos] = rchar;
...
ssize_t send_oob_with_ttl(int sfd, char *buffer,
        ssize_t n, long pos, int fa, char *c, int ttl)
{
    if (ttl <= 0) {
        return send_oob(sfd, buffer, n, pos, c);
    }
        ...
        char *buffer, long pos, int fa)
{
    ...
    ssize_t len = send_full(sfd, buffer, pos, 0);
    if (len < 0) {
        uniperror("send");
    }
...
ssize_t send_late_oob(int sfd, char *buffer,
        ssize_t n, long pos, int fa, char *c, int ttl)
{
    return send_oob_with_ttl(sfd, buffer, n, pos, fa, c, ttl);
}


ssize_t desync(int sfd, char *buffer, size_t bfsize,
        ssize_t n, ssize_t offset, struct sockaddr *dst, int dp_c,
        int *sni_profile, int *sni_index)
{
    struct desync_params dp = params.dp[dp_c];
    const char *selected_sni = 0;
    if (dp.sni_count > 0) {
        if (*sni_profile != dp_c || *sni_index < 0 || *sni_index >= dp.sni_count) {
            *sni_profile = dp_c;
            *sni_index = rand() % dp.sni_count;
            LOG(LOG_S, "fake SNI selected: %d/%d\n", *sni_index + 1, dp.sni_count);
        }
        selected_sni = dp.sni_list[*sni_index];
    }
    ...
    if (type == IS_HTTP && dp.mod_http) {
        ...
        for (int i = 0; i < dp.tlsrec_n; i++) {
            ...
            long pos = (part.pos_range
                ? random_between(part.pos_min, part.pos_max)
                : part.pos) + i * 5;
        ...
        switch (part.m) {
            ...
            case DESYNC_FAKE:
                if (pos != lp) s = send_fake(sfd,
                    buffer + lp, type, pos - lp, fa, &dp, selected_sni);
                break;
            #endif
            ...
            case DESYNC_OOB:
                s = send_oob_with_ttl(sfd,
                    buffer + lp, n - lp, pos - lp, fa, dp.oob_char,
                    dp.oob_ttl_set ? dp.oob_ttl : 0);
                break;
                
            case DESYNC_DISOOB:
                s = send_late_oob(sfd,
                    buffer + lp, n - lp, pos - lp, fa, dp.oob_char,
                    dp.oob_ttl_set ? dp.oob_ttl : 1);
                break;
                
            case DESYNC_SPLIT:
            case DESYNC_NONE:
                s = send_full(sfd, buffer + lp, pos - lp, 0);
                wait_send_if_support(sfd);
                break;
    ...
    if (lp < n) {
        ...
        ssize_t sent = send_full(sfd, buffer + lp, n - lp, 0);
        if (sent < 0) {
            if (get_e() == EAGAIN) {
                return lp;
            }
