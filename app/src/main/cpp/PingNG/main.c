...
const char help_text[] = {
    ...
    "    -R, --split-range <a-b[+s]>  Pick a bounded split position\n"
    "                              +s - add SNI offset\n"
    ...
    "    -j, --oob-ttl <num>       TTL of OOB packets, 0 uses system default\n"
    #ifdef FAKE_SUPPORT
    "    -f, --fake <n[+s]>        Split and send fake packet\n"
    "    -t, --ttl <num>           TTL of fake packets, default 8\n"
    "    -Q, --ttl-range <a-b>     Pick a bounded fake TTL\n"
    #ifdef __linux__
    ...
    "    -J, --fake-jitter         Randomise valid TLS padding in fake ClientHello\n"
    #endif
    ...
    "    -Z, --delay-range <a-b>   Pick a bounded handshake delay\n"
    #ifdef __linux__
...
const int options_count = sizeof(options) / sizeof(*options);
...
int parse_offset_range(struct part *part, const char *str)
{
    ...
    if (!copy) return -1;
    char *suffix = strchr(copy, '+');
    if (suffix) {
        *suffix++ = 0;
        if (*suffix == 's') part->flag = OFFSET_SNI;
        else if (*suffix == 'h') part->flag = OFFSET_HOST;
        else if (*suffix == 'e') part->flag = OFFSET_END;
        else { free(copy); return -1; }
    }
    char *dash = strchr(copy, '-');
    if (!dash || dash == copy || !dash[1]) { free(copy); return -1; }
    *dash++ = 0;
    char *end = 0;
    long min = strtol(copy, &end, 0);
    if (*end) { free(copy); return -1; }
    long max = strtol(dash, &end, 0);
    if (*end || min < 1 || max < min || max > 0xffff) {
        free(copy);
        return -1;
    }
    part->pos = min;
        ...
        int *min_out, int *max_out)
{
    ...
    *dash++ = 0;
    ...
    if (*end) { free(copy); return -1; }
    long max = strtol(dash, &end, 0);
    int invalid = *end || min < min_allowed || max < min || max > max_allowed;
    free(copy);
    if (invalid) return -1;
    *min_out = (int)min;
    *max_out = (int)max;
...
int main(int argc, char **argv) 
{
    ...
    while (!invalid && (rez = getopt_long(
        ...
        case 'R':
            ;
            struct part *range_part = add((void *)&dp->parts,
                &dp->parts_n, sizeof(struct part));
            if (!range_part) {
                clear_params();
                return -1;
            }
            range_part->m = DESYNC_SPLIT;
            if (parse_offset_range(range_part, optarg)) invalid = 1;
            break;
        ...
        case 't':
            ...
            if (val <= 0 || val > 255 || *end) 
                ...
                dp->ttl = dp->ttl_min = dp->ttl_max = val;
            break;

        case 'Q':
            if (parse_int_range(optarg, 1, 255, &dp->ttl_min, &dp->ttl_max)) {
                invalid = 1;
            } else {
                dp->ttl = dp->ttl_min;
            }
        ...
        case 'r':
            ...
            if ((strchr(optarg, '-')
                    ? parse_offset_range(part, optarg)
                    : parse_offset(part, optarg))
                   || part->pos > 0xffff) {
                invalid = 1;
                break;
            }
        ...
        case 'w': //
            ...
            if (params.sfdelay < 0 || optarg == end 
                ...
                params.sfdelay_min = params.sfdelay_max = params.sfdelay;
            break;

        case 'Z': { //
            int delay_min = 0;
            int delay_max = 0;
            if (parse_int_range(optarg, 0, 999, &delay_min, &delay_max)) {
                invalid = 1;
            } else {
                params.sfdelay_min = delay_min;
                params.sfdelay_max = delay_max;
                params.sfdelay = params.sfdelay_min;
            }
            break;
        }
