...
object PingNgDesyncManager {
    ...
    fun start(profile: ProfileItem): Int {
        return startLocked(profile, 0)
    }

    /** Replaces only the native listener while the proxy-only Xray stays up. */
    @Synchronized
    fun reconfigure(profile: ProfileItem): Int {
        val existingPort = port
        if (existingPort !in 1..65535) return start(profile)
        return startLocked(profile, existingPort)
    }

    private fun startLocked(profile: ProfileItem, preferredPort: Int): Int {
        stopLocked()
        ...
        val selectedPort = preferredPort.takeIf { it in 1..65535 }
            ?: Utils.findRandomFreePort()
    ...
    private fun stopLocked() {
        ...
        // The native parser uses process-global parameters. Wait for the old
        // event loop to leave before parsing the next search candidate, so a
        // previous Split profile cannot leak into the next method.
        if (oldWorker != null && oldWorker !== Thread.currentThread()) {
            runCatching { oldWorker.join(500L) }
        }
    }
}
