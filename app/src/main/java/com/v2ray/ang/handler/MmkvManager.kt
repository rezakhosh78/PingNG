...
object MmkvManager {
    ...
    fun encodeServerConfig(guid: String, config: ProfileItem): String {
        ...
        withProfileIndexLock {
            // Keep subscription ownership when an editor/parser omitted the
            // field. Without this, a perfectly valid edit (for example
            // enabling Psiphon) is saved once but is lost on the next refresh.
            val previous = decodeServerConfig(key)
            if (config.subscriptionId.isBlank() && !previous?.subscriptionId.isNullOrBlank()) {
                config.subscriptionId = previous?.subscriptionId.orEmpty()
            }
            requireStorageWrite(
    ...
    internal fun saveServerProfiles(
        ...
        withProfileIndexLock {
            ...
            val replacementSelection = ProfileReplacement.findSelectedReplacement(
                profiles = profilesToPersist,
                currentSelection = previousSelection,
                selectedProfile = selectedProfile,
            )

            profilesToPersist.forEach { (guid, profile) ->
                requireStorageWrite(
                    profileFullStorage.encode(guid, JsonUtil.toJson(profile)),
                    "Failed to save profile payload",
                )
                rawConfigsByPersistedGuid[guid]?.let { raw ->
                    requireStorageWrite(
            ...
            val removablePayloads = ProfileReplacement.findRemovablePayloads(
                ...
                replacementServers = profilesToPersist.keys,
