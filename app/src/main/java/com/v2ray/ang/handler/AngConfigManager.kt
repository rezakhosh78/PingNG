    ...
    fun updateConfigViaSub(it: SubscriptionCache): SubscriptionUpdateResult {
        try {
            ...
            var response = try {
                ...
                HttpUtil.getUrlContentWithUserAgentAndHeaders(
                    UrlContentRequest(
                        url = url,
                        userAgent = userAgent,
            ...
            // Persist provider metadata as soon as the HTTP response is valid.
            // Parsing the server list and displaying the notice are independent:
            // a provider may return a notice even when its payload contains no
            // currently usable profiles.
            response?.let { httpResponse ->
                ...
                SubscriptionNoticeParser.extractTrafficUsage(httpResponse, it.subscription.url)?.let { usage ->
                    ...
                    it.subscription.trafficUsedBytes = usage.usedBytes
                    it.subscription.expirationEpochSeconds = usage.expirationEpochSeconds
                    it.subscription.trafficTotalRequests = usage.totalRequests
                    it.subscription.trafficUsedRequests = usage.usedRequests
                } ?: run {
                    it.subscription.trafficTotalBytes = -1
                    it.subscription.trafficUsedBytes = -1
                    it.subscription.expirationEpochSeconds = -1
                    it.subscription.trafficTotalRequests = -1
                    it.subscription.trafficUsedRequests = -1
                }
                it.subscription.notice = SubscriptionNoticeParser.extract(httpResponse)
