...
object HttpUtil {
    ...
    /** Retrieves URL content through the running local SOCKS inbound. */
    fun getUrlContentThroughSocks(request: UrlContentRequest, socksPort: Int): String? {
        val url = request.url ?: return null
        if (socksPort !in 1..65535) return null

        val client = OkHttpClient.Builder()
            .proxy(Proxy(Proxy.Type.SOCKS, InetSocketAddress(LOOPBACK, socksPort)))
        ...
        return try {
            client.newCall(requestBuilder.build()).execute().use { response ->
                if (!response.isSuccessful) {
                    LogUtil.w(AppConfig.TAG, "Failed to get URL content through SOCKS, code=${response.code}")
                    return null
                }
                response.body?.string()
            }
        } catch (e: Exception) {
            LogUtil.w(AppConfig.TAG, "SOCKS URL content probe failed: ${e.message}")
            null
        }
    }

    /** Measures a URL through the running local SOCKS inbound. */
    fun measureUrlDelayThroughSocks(request: UrlContentRequest, socksPort: Int): Long {
        val url = request.url ?: return -1L
        if (socksPort !in 1..65535) return -1L
        ...
        return try {
            // Any HTTP response proves that the live tunnel completed the round-trip;
            // the probe URL need not return a 2xx status to measure its delay.
            client.newCall(requestBuilder.build()).execute().use { _ ->
                (System.currentTimeMillis() - startedAt).coerceAtLeast(0L)
            }
        } catch (e: Exception) {
            LogUtil.w(AppConfig.TAG, "Live SOCKS delay probe failed: ${e.message}")
            -1L
        }
    }
    ...
    fun getUrlContentWithUserAgent(request: UrlContentRequest): String {
        return getUrlContentWithUserAgentAndHeaders(request).content
    }

    /**
     * Retrieves subscription content and preserves the final response headers.
     * Redirect responses are followed manually so metadata from the final provider
     * response is the metadata stored for the subscription.
     */
    @Throws(IOException::class)
    fun getUrlContentWithUserAgentAndHeaders(request: UrlContentRequest): UrlContentResponse {
        ...
        while (redirects++ < maxRedirects) {
            ...
            val finalUserAgent = if (request.userAgent.isNullOrBlank()) {
                "PingNG/${BuildConfig.VERSION_NAME}"
            } else {
                request.userAgent
            }
            val requestBuilder = Request.Builder()
    ...
    private fun buildOkHttpClient(
        ...
        val externalProxy = if (httpPort == 0) SettingsManager.getConnectHttpProxy() else null
        val proxyAddress = when {
            httpPort != 0 -> InetSocketAddress(LOOPBACK, httpPort)
