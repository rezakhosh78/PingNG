...
class ServerActivity : BaseComponentActivity() {
    ...
    private fun runFinalMaskSearch(
        ...
        finalMaskSearchJob = (application as com.v2ray.ang.AngApplication).applicationScope.launch(Dispatchers.IO) {
            ...
            try {
                withContext(Dispatchers.Main) { LauncherManager.stopService(this@ServerActivity) }
                kotlinx.coroutines.delay(700L)
                candidates.forEachIndexed { index, candidate ->
                    try {
                        MmkvManager.encodeServerConfig(guid, original.copy(finalMask = candidate.json))
                        withContext(Dispatchers.Main) {
                            if (index == 0) LauncherManager.startProxyOnlyService(this@ServerActivity, guid)
                            else {
                                LauncherManager.stopService(this@ServerActivity)
                                kotlinx.coroutines.delay(350L)
                                LauncherManager.startProxyOnlyService(this@ServerActivity, guid)
                            }
                        }
                ...
                withContext(NonCancellable + Dispatchers.Main) {
                    LauncherManager.stopService(this@ServerActivity)
                    if (isRunning) LauncherManager.startService(this@ServerActivity, guid)
                    onFinished()
                }
            }
...
fun ServerScreen(
    ...
    onFinalMaskSearch: (
        ProfileItem,
        List<FinalMaskCandidate>,
        (FinalMaskCandidate, Long) -> Unit,
        (Int) -> Unit,
        () -> Unit,
    ) -> Unit,
