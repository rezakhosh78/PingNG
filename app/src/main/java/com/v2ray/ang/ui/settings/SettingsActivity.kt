...
fun SettingsScreen(
    ...
    var connectHttpProxy by rememberMmkvBool(AppConfig.PREF_CONNECT_HTTP_PROXY, false)
    var connectHttpProxyHost by rememberMmkvString(AppConfig.PREF_CONNECT_HTTP_PROXY_HOST, "")
    var connectHttpProxyPort by rememberMmkvString(AppConfig.PREF_CONNECT_HTTP_PROXY_PORT, "8080")
    var connectHttpProxyUsername by rememberMmkvString(AppConfig.PREF_CONNECT_HTTP_PROXY_USERNAME, "")
    var connectHttpProxyPassword by rememberMmkvString(AppConfig.PREF_CONNECT_HTTP_PROXY_PASSWORD, "")
    ...
    Scaffold(
        ...
        Column(
            ...
            if (coreSettingsExpanded) {
                ...
                SettingsSwitchItem(
                    title = stringResource(R.string.title_pref_connect_http_proxy),
                    summary = stringResource(R.string.summary_pref_connect_http_proxy),
                    checked = connectHttpProxy,
                    onCheckedChange = { connectHttpProxy = it }
                )
                if (connectHttpProxy) {
                    SettingsEditItem(
                        title = stringResource(R.string.title_pref_connect_http_proxy_host),
                        value = connectHttpProxyHost,
                        onValueChanged = { connectHttpProxyHost = it.trim() }
                    )
                    SettingsEditItem(
                        title = stringResource(R.string.title_pref_connect_http_proxy_port),
                        value = connectHttpProxyPort,
                        keyboardNumber = true,
                        onValueChanged = { value ->
                            connectHttpProxyPort = value.filter(Char::isDigit).take(5)
                        }
                    )
                    SettingsEditItem(
                        title = stringResource(R.string.title_pref_connect_http_proxy_username),
                        value = connectHttpProxyUsername,
                        onValueChanged = { connectHttpProxyUsername = it }
                    )
                    SettingsEditItem(
                        title = stringResource(R.string.title_pref_connect_http_proxy_password),
                        value = connectHttpProxyPassword,
                        isPassword = true,
                        onValueChanged = { connectHttpProxyPassword = it }
                    )
                }
                SettingsSwitchItem(
