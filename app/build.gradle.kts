...
if (!libV2rayFile.isFile || libV2rayFile.length() <= 1024) {
    libV2rayFile.parentFile.mkdirs()
    val temporary = libV2rayFile.resolveSibling("${libV2rayFile.name}.download")
    URI(
        "https://github.com/2dust/AndroidLibXrayLite/releases/download/v26.8.20/libv2ray.aar"
    ).toURL().openStream().use { input ->
        temporary.outputStream().use { output -> input.copyTo(output) }
    }
    check(temporary.length() > 1024 && temporary.inputStream().use { input ->
        input.read() == 'P'.code && input.read() == 'K'.code
    }) { "Downloaded libv2ray.aar is invalid" }
    temporary.copyTo(libV2rayFile, overwrite = true)
    temporary.delete()
}
